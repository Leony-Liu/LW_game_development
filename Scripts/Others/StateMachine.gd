...
func _ready() -> void:
	# 稍微延迟一帧初始化，确保父节点（Player/Enemy）以及所有组件都已经准备好
	await owner.ready 
	...
	for child in get_children():
		if child is State:
			...
			# 这一步非常关键：把场景的根节点（也就是 Player 或 Enemy 控制器）传给状态
			# 这样状态脚本里就能直接调用 player.anim_player.play() 了
			child.player = owner 
